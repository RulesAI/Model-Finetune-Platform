...
| 数据标注平台 | 集成 Argilla |
| 基础模型预训练 | 不在范围 |
| 多模态训练（VLM，视觉+语言**融合**）| v2+ 再考虑 |
| 纯视觉 CV（YOLO/SAM/检测/分割，仅视觉输入）| 独立训练线，不在本平台 LLM 微调栈（ms-swift）内 |

> **模型分三桶（边界澄清，详见 `docs/10` §1.1）**：① 文本 LLM＝本平台主业；② 多模态/VLM（视觉+语言融合）＝上表 v2+；③ 纯视觉 CV（YOLO/SAM，仅视觉输入）＝**第三类，既非 ① 也非 ②**。"CV"是任务域、不是模型类；架构≠模态数（ViT 用 Transformer 仍纯视觉）。**「不做多模态」≠「已排除 CV」**——纯视觉是否单开训练线，需独立立项决策，勿默认并入"多模态"一谈。
